...
Select
    ...
    year(curdate()) - year(EMP_DOB) as Age
from
    Professor
order by
    EMP_DOB asc
...
Select
    Round(AVG(STU_GPA), 2) as "Average CIS GPA"
from
    Student
where
    DEPT_CODE = "CIS";
...
SELECT
    COUNT(*) AS "Total # of 3-Credit Courses"
FROM
    Course
WHERE
    DEPT_CODE IN ("CIS", "MATH", "BIOL", "ENG")
    AND CRS_CREDIT = 3;

-- Q4 Find the total number of students in each department.
SELECT
    DEPT_CODE,
    COUNT(*) AS "Total # of Students"
FROM
    Student
WHERE
    DEPT_CODE IN ('CIS', 'MATH', 'BIOL', 'ENG')

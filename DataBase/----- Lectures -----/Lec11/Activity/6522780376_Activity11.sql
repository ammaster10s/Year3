...
-- Add foreign key on class.EMP_NUM referencing professor.EMP_NUM
ALTER TABLE `class`
  ADD CONSTRAINT `fk_class_professor`
  FOREIGN KEY (`EMP_NUM`) REFERENCES `professor` (`EMP_NUM`);

-- Add foreign key on course.DEPT_CODE referencing department.DEPT_CODE
ALTER TABLE `course`
  ADD CONSTRAINT `fk_course_department`
  FOREIGN KEY (`DEPT_CODE`) REFERENCES `department` (`DEPT_CODE`);

-- Add foreign key on department.EMP_NUM referencing professor.EMP_NUM
ALTER TABLE `department`
  ADD CONSTRAINT `fk_department_professor`
  FOREIGN KEY (`EMP_NUM`) REFERENCES `professor` (`EMP_NUM`);

-- Add foreign key on enroll.CLASS_CODE referencing class.CLASS_CODE
ALTER TABLE `enroll`
  ADD CONSTRAINT `fk_enroll_class`
  FOREIGN KEY (`CLASS_CODE`) REFERENCES `class` (`CLASS_CODE`);

-- Add foreign key on enroll.STU_NUM referencing student.STU_NUM
ALTER TABLE `enroll`
  ADD CONSTRAINT `fk_enroll_student`
  FOREIGN KEY (`STU_NUM`) REFERENCES `student` (`STU_NUM`);

-- Add foreign key on professor.DEPT_CODE referencing department.DEPT_CODE
ALTER TABLE `professor`
  ADD CONSTRAINT `fk_professor_department`
  FOREIGN KEY (`DEPT_CODE`) REFERENCES `department` (`DEPT_CODE`);

-- Add foreign keys on student table
ALTER TABLE `student`
...
-- Q1 Insert Data to table professors
insert into professor values ( 101, "Charles" , "Xavier", "1975,5,4", "CIS", Null);
-- Q2 Update Data to table professors
Update professor set PROF_EXTENSION = '0077' where EMP_NUM = 101;
-- Q3 Delete Professor Chrales
Delete from professor where EMP_NUM = 101;

select * from professor;

-- Q4 Show all student's ID fullname, birthdate, and age 

Select STU_NUM, concat(STU_FNAME," ", STU_LNAME) as Fullname, STU_DOB, year(curdate()) - year(STU_DOB) as Age from student; 

-- Q5 List all unique. course code from the Class table

Select Distinct COURSE_CODE from class ; 

-- Q6 List all CIS professor names (fname and lname, department code )

Select EMP_FNAME, EMP_LNAME, DEPT_CODE from Professor where DEPT_CODE = "CIS";

-- Q7 List all professors (First name and last name) who have the letter "M" in his/her first name and last name

SELECT EMP_FNAME, EMP_LNAME 
FROM Professor 
WHERE EMP_FNAME LIKE '%M%' 
  AND EMP_LNAME LIKE '%M%';
  
-- Q8  Show the list of student who are studying at the department ...

SELECT STU_FNAME ,STU_LNAME, DEPT_CODE from student where DEPT_CODE in ("ENG", "ART","SOC", "HIST" );

-- Q9 Show the amount of tuition fee of all intro cost...

SELECT COURSE_CODE, COURSE_NAME, CRS_CREDIT*250 as Tuition_fee ,DEPT_CODE from course where COURSE_NAME like "Intro%" ORDER by DEPT_CODE;

-- Q10 Select course that are locate in the classroom beginning with KLR and have class code between 20000 and 30000 inclusively 
